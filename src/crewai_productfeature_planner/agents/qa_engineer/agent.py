"""QA Engineer agent factory.

Creates a QA Engineer agent that generates testing sub-task tickets
as counter-tickets to development sub-tasks, focusing on edge cases,
security testing, and scenarios beyond unit tests.
"""
...
def _build_llm() -> LLM:
    ...
    logger.info(
        "QA Engineer LLM: %s (timeout=%ds, retries=%d)",
...
def create_qa_engineer(
    ...
    """Create a QA Engineer agent with Jira tool."""
    has_api_key = bool(os.environ.get("GOOGLE_API_KEY"))
    has_project = bool(os.environ.get("GOOGLE_CLOUD_PROJECT"))
    if not has_api_key and not has_project:
        raise EnvironmentError(
            "QA Engineer requires GOOGLE_API_KEY or "
            "GOOGLE_CLOUD_PROJECT to be set."
        )

    agent_config = _load_yaml("agent.yaml")["qa_engineer"]
    logger.info(
        "Creating QA Engineer agent (role='%s')",
