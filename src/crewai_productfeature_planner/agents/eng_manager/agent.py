"""Eng Manager agent factory.

Creates an Engineering Manager / Technical Architect agent that reviews
executive product summaries and produces engineering plans for Jira
ticket creation. Uses the research model tier for deep technical thinking.
"""
...
def _build_llm(provider: str = PROVIDER_GEMINI) -> LLM:
    """Build the LLM for the Eng Manager agent.

    Uses the research model tier — engineering planning requires deep
    technical reasoning about architecture and failure modes.
    """
...
def create_eng_manager(
    ...
    if provider == PROVIDER_GEMINI:
        ...
        if not has_api_key and not has_project:
            raise EnvironmentError(
                "Eng Manager requires GOOGLE_API_KEY or "
                "GOOGLE_CLOUD_PROJECT to be set."
            )

    agent_config = _load_yaml("agent.yaml")["eng_manager"]
    logger.info(
        "Creating Eng Manager agent (provider='%s', role='%s')",
    ...
    return Agent(
        ...
        tools=[],  # Eng planning is pure text reasoning — no tools needed
        verbose=is_verbose(),
...
def get_task_configs() -> dict:
    """Load and return the Eng Manager task configurations."""
