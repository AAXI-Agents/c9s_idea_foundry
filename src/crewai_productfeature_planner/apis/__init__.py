...
async def _lifespan(application: FastAPI):
    ...
    # 2b. Mark unfinalized working ideas as failed.
    terminated_ideas: list[dict] = []
    try:
        from crewai_productfeature_planner.mongodb.working_ideas.repository import (
            find_resumable_on_startup,
        )
        _resumable, terminated_ideas = find_resumable_on_startup()
        # Previously resumable ideas were auto-resumed via Slack;
        # now they are also marked failed (resume via API/UI instead).
        terminated_ideas.extend(_resumable)
        if terminated_ideas:
            _logger.info(
